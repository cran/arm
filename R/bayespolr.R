...
function (formula, data, weights, start, ..., subset, na.action, 
    ...
    prior.df=1, scaled=TRUE,
        prior.mean.for.cutpoints=0, prior.scale.for.cutpoints=10,
       prior.df.for.cutpoints=1)
{
    logit <- function(p) log(p/(1 - p))
    dt.deriv <- function (x, mean, scale, df, log=TRUE, delta=.001){
      (dt ((x+delta-mean)/scale, df, log=log) - dt ((x-delta-mean)/scale, df, log=log))/(2*delta)
    }
    fmin <- function(beta) {
        theta <- beta[pc + 1:q]
        ...
        if (all(pr > 0))
          ...
          f <- Inf
# Now put in the prior information
        zeta <- cumsum(c(theta[1], exp(theta[-1])))
        zeta.centered <- zeta - sum (beta[1:pc]*colMeans(x))  # 17 Dec
        f <- f - sum (dt ((zeta.centered - prior.mean.for.cutpoints)/prior.scale.for.cutpoints, prior.df.for.cutpoints, log=TRUE))
        if (pc > 0) 
          f <- f - sum (dt ((beta[1:pc] - prior.mean)/prior.scale, prior.df, log=TRUE))
        return (f)
    }
    gmin <- function(beta) {
...
# Add deriv of the log prior dist to the gradient
# (for simplicity, just use numerical deriv of the t density)
        zeta <- cumsum(c(theta[1], exp(theta[-1])))
        zeta.centered <- zeta - sum (beta[1:pc]*colMeans(x))  # 17 Dec
        g2 <- g2 - dt.deriv (zeta.centered, prior.mean.for.cutpoints, prior.scale.for.cutpoints, prior.df.for.cutpoints, log=TRUE)
        if (pc > 0)
          g1 <- g1 - dt.deriv (beta[1:pc], prior.mean, prior.scale, prior.df, log=TRUE)
        if (all(pr) > 0) 
            c(g1, g2)
        else rep(NA, pc + q)
    }
    ...
    if (missing(start)) {
        ...
        fit <- switch(method,
                      logistic = bayesglm.fit(X, y1, wt, family = binomial(), offset = offset, intercept=TRUE, prior.mean=prior.mean, prior.scale=prior.scale, prior.scale.for.intercept=10, prior.df=prior.df, scaled=scaled),
                      probit = bayesglm.fit(X, y1, wt, family = binomial("probit"), offset = offset, intercept=TRUE, prior.mean=prior.mean, prior.scale=prior.scale, prior.scale.for.intercept=10, prior.df=prior.df, scaled=scaled),
                      cloglog = bayesglm.fit(X, y1, wt, family = binomial("probit"), offset = offset, intercept=TRUE, prior.mean=prior.mean, prior.scale=prior.scale, prior.scale.for.intercept=10, prior.df=prior.df, scaled=scaled),
                      cauchit = bayesglm.fit(X, y1, wt, family = binomial("cauchit"), offset = offset, intercept=TRUE, prior.mean=prior.mean, prior.scale=prior.scale, prior.scale.for.intercept=10, prior.df=prior.df, scaled=scaled))
        if (!fit$converged) 
...
# Initialize the parameters in the prior distribution:
#   chunk of code copied from bayesglm.fit()
# If only one set of prior parameters is set, expand to all the coefs
  ...
  # Above lines are commented out since polr() handles intercepts separately
    if (length(prior.df)==1) prior.df <- rep (prior.df, J)
  # New lines for expanding cutpoint parameters
